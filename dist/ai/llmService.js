...
function getClient() {
    if (!client) {
        client = new openai_1.default({
            apiKey: env_1.config.llmApiKey || 'dummy-key',
 ...
 */
function analyzeCreativeInterests(prompts) {
    if (prompts.length === 0) {
...
async function generateEmail(user) {
    try {
        ...
        const systemPrompt = `You are Aayushi, a friendly team member at ${env_1.config.brandName} (https://www.artnovaai.com). Your job is to write compelling, authentic emails to creators about specific challenges they face and how to solve them.
...
5. Mention that ${env_1.config.brandName} solves this specific problem NATURALLY (as a helpful friend would).
...
5. Position ${env_1.config.brandName} as the natural, elegant solution
...
Return ONLY a JSON object with this exact structure:
{
  ...
  "body": "the email body as engaging plain text"
        ...
        try {
            ...
            logger_1.logger.warn(`JSON parse failed for ${user.email}, retrying...`);
        ...
        logger_1.logger.error(`Failed to generate email for ${user.email}: ${error instanceof Error ? error.message : String(error)}`);
        throw error;
    }
}
/**
